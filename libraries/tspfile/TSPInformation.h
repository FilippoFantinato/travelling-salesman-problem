...
namespace TSPFile
{
    ...
    std::ostream& operator<<(std::ostream& os, const TSPInformation& info)
    {
        ...
        os << "OPTIMAL_SOLUTION: ";
        if(info.optimal_solution != -1) {os << info.optimal_solution;} else {os << "N/A";}
    ...
    std::shared_ptr<const TSPInformation> read_information(std::ifstream &ifd)
    {
        ...
        bool node_coord_section_met = false;

        std::string line;
        while(
            getline(ifd, line) && 
            !(node_coord_section_met = line.compare(TSPSection::node_coord_section)==0))
        {
        ...
        if(!node_coord_section_met) throw IncompleteTSPFile(TSPSection::node_coord_section);
