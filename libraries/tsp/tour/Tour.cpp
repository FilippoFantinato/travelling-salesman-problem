...
Tour::Tour(const std::vector<Vertex>& tour) : tour(tour)
{
...
{
    std::vector<VertexPair> tmp(cbegin(), cend());
    std::vector<VertexPair> edges;

    std::set_difference(tmp.begin(), tmp.end(), broken.begin(), broken.end(), edges.begin());
    edges.insert(edges.end(), joined.begin(), joined.end());

    if(edges.size() < size()) return NULL;
    ...
    std::vector<Vertex> *new_tour = NULL;
